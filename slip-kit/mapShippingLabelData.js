// slip-kit/mapShippingLabelData.js
//
...
// (as data URLs). Node port of the original frontend mapper.
    ...
    orders.map(async (order) => {
      ...
      const orderLabels = await Promise.all(
        fulfillmentOrders.map(async (fulfillment, idx) => {
          ...
          // Round COD to a whole number — it drives the on-slip display, the
          // cod_amount barcode, AND the QR payload, so all three stay in sync.
          const codRounded = Math.round(parseFloat(fulfillment.cod_amount ?? 0) || 0);
          const cod_str = String(codRounded);
          const productDetails = shipmentItems
            .map((item) => `${item.sku || item.name || 'Item'} x ${item.quantity || 0}`)
            .join(', ');
          ...
          const { codBarcodeText, codBarcodeWithText, qrPayload, extraBarcodeText } =
            strategy({ fulfillment, order, courierCity, shipperDetails, cod_str });
